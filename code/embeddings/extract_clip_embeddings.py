...
def get_clip_layer_representations(args, text_array, word_ind_to_extract):
    ...
    model = CLIPModel.from_pretrained(model_hf_path, cache_dir="./").to(device)
    tokenizer = CLIPTokenizer.from_pretrained(model_hf_path, cache_dir="./")
    model.eval()

    # get the token embeddings
    token_embeddings = []
    for word in text_array:
        current_token_embedding = get_clip_token_embeddings([word], tokenizer, model)
    ...
    for truncated_seq_len in range(1, 1+seq_len):
        ...
        Words_layers_representations = add_avrg_token_embedding_for_specific_word_clip(word_seq, tokenizer, model, 
...
@torch.inference_mode()
def predict_clip_embeddings(words_in_array, tokenizer, model, model_config):    
    n_seq_tokens = 0
    seq_tokens = []
    ...
    # For CLIP, we use the text encoder
    outputs = model.text_model(tokens_tensor, output_hidden_states=True)
    hidden_states = outputs['hidden_states'][1:]    # This is a tuple: (layer1, layer2, ..., layer12)
...
@torch.inference_mode()
def add_word_clip_embedding(model_dict, embeddings_to_add, token_inds_to_avrg, specific_layer=-1):
...
@torch.inference_mode()
def add_avrg_token_embedding_for_specific_word_clip(word_seq,tokenizer,model,from_start_word_ind_to_extract,model_dict, model_config):
    
    word_seq = list(word_seq)
    all_sequence_embeddings, word_ind_to_token_ind, _ = predict_clip_embeddings(word_seq, tokenizer, model, model_config)
    token_inds_to_avrg = word_ind_to_token_ind[from_start_word_ind_to_extract]
    model_dict = add_word_clip_embedding(model_dict, all_sequence_embeddings,token_inds_to_avrg)
    
    return model_dict

# get the model token embeddings
@torch.inference_mode()
def get_clip_token_embeddings(words_in_array, tokenizer, model):    
    n_seq_tokens = 0
    seq_tokens = []
    ...
    # For CLIP, get input embeddings from text model
    input_embedding_module = model.text_model.embeddings.token_embedding
...
if __name__ == "__main__":
    ...
    for eachstory in story_files:
        ...
        embeddings = get_clip_layer_representations(args, np.array(words), word_ind_to_extract)
